...
def get_flag(tick: int) -> str:
    ...
    if _launched_without_runner():
        ...
        return lib.flag.generate(expiration, 42, team, b'1')
...
def run_check(checker_cls: Type[BaseChecker]) -> None:
    ...
    # Get service name from environment or use placeholder
    service = os.environ.get('CTF_SERVICE', 'unknown')
    ...
    # Lock to ensure logs for one tick are printed together without interleaving
    if _ctrl_out_lock:
        _ctrl_out_lock.acquire()
    
    try:
        # Print header for this tick
        result = _run_check_steps(checker, tick)
        
        # Print footer with final status
        if result == CheckResult.OK:
            service_status = "UP"
        elif result == CheckResult.DOWN:
            service_status = "DOWN"
        else:
            service_status = str(result)
        
        logging.info('===============================================')
        logging.info('TICK: %d | TEAM: %d | SERVICE: %s | IP: %s', tick, team, service, ip)
        logging.info('===============================================')
        logging.info('FINAL STATUS: %s', service_status)
        logging.info('===============================================')
    finally:
        if _ctrl_out_lock:
            _ctrl_out_lock.release()
...
def _run_check_steps(checker, tick):
    ...
    try:
        ...
        # Check the flag of the current tick
        result = checker.check_flag(tick)
        if result != CheckResult.OK:
            return result
        else:
