...
VPN_CONFIG_DIR = './data/vpn-config/testbox'
...
def update_vpn_team_path(db_conn, team_id, file_path, ip_address):
    ...
    try:
        result = db_conn.query(
            'UPDATE "vpn_teams" SET path = %s, "ipVpn" = %s WHERE "idTeam" = %s AND "typeVpn" = false',
        ...
        if result and result > 0:
            ...
            print(f"  ⚠ No vpn_teams record updated for team {team_id} (typeVpn=false)")
            return False
    except Exception as e:
...
try:
    existing_clients_response = session.get(URL_WIREGUARD + E_CRE_TEAMS)
...
# First pass: Create VPN clients with _test suffix
print("Creating VPN clients...")
for team in teams:
    team_id, team_slug = team[0], team[1]
    testbox_name = team_slug + "_test"
    
    # Check if client already exists
    if testbox_name.lower() in existing_names:
        print(f"  ⊘ Already exists: {testbox_name}")
        continue
    
    r = session.post(
        URL_WIREGUARD + E_CRE_TEAMS,
        json={'name': testbox_name, 'expiresAt': ExpiresAt}
    )
    if r.status_code == 201:
        print(f"  ✓ Created: {testbox_name}")
    else:
        print(f"  ⚠ Status {r.status_code}: {testbox_name}")
...
for team in teams:
    team_id, team_slug = team[0], team[1]
    testbox_name = team_slug + "_test"
    print(f"Processing team: {testbox_name} (ID: {team_id})")
    ...
    for client in clients:
        ...
        # Check if client name matches testbox name
        if testbox_name.lower() in client_name.lower():
    ...
    for client in matched_clients:
        ...
        try:
            ...
            file_path = save_vpn_config(config_text, testbox_name, team_id)
            if not file_path:
                continue
            
            # Update database with file path and IP (only for typeVpn=false)
...
db.disconnect()
print("✓ VPN testbox sync completed")
