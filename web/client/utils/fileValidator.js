 ...
 * @param {string} filePath - File path to delete (can be full path or just filename)
 * @returns {boolean} - Success status
 */
const deleteUploadedFile = (uploadDir, filePath) => {
    try {
        ...
        // Verify path is safe
        if (!isPathSafe(uploadDir, fullPath)) {
            console.error(`[DELETE] Path safety check failed: ${fullPath}`);
            return false;
        }

        if (fs.existsSync(fullPath)) {
            fs.unlinkSync(fullPath);
            console.log(`[DELETE] Successfully deleted: ${fullPath}`);
            return true;
        } else {
            console.warn(`[DELETE] File does not exist: ${fullPath}`);
            return false;
        }
    } catch (error) {
